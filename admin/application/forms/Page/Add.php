<?php 
class Application_Form_Page_Add extends Zend_Form {
	...
	public function init() {
		...
		$this->setName('edit');
		$this->setAction($view->baseUrl('page/add'));
		$this->setMethod('post');
		$translate = Zend_Registry::get('translate');
		
		$seo_url = new Zend_Form_Element_Text('seo_url');
		$seo_url->setLabel($translate->_('SEO url'));
		...
		$template->setLabel($translate->_('Template'));
		...
		$active->setLabel($translate->_('Active'));
		...
		foreach($languages as $language) {
			...
			$meta_keywords->setLabel($translate->_('meta_keywords'));
			...
			$meta_description->setLabel($translate->_('meta_description'));
			...
			$description->setLabel($translate->_('Text'));
		...
		$this->addElements(array($seo_url, $template, $active, $submit));
	}
}