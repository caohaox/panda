(function($) {
	...
	function allinone_carousel_navigation(direction,options,current_obj,bottomNavButs,imgs,allinone_carousel_container,contentHolderUnitOrigWidth,contentHolderUnitOrigHeight,total_images,allinone_carousel_playOver,allinone_carousel_elementTitle){
			...
			// begin
			var chinaCity = { 
						  Chengdu 	: { e: 'Chengdu', z: '成都', r : 'Чэнду'} ,
						  Xian 		: { e: 'Xian', z: '西安', r : 'Сиань'} ,
						  Beijing 	: { e: 'Beijing', z: '北京', r : 'Пекин'} , 
						  Guangzhou : { e: 'Guangzhou', z: '广州', r : 'Гуанчжоу'} ,
						  Shanghai 	: { e: 'Shanghai', z: '上海', r : 'Шанхай'} , 
						  Nanjing 	: { e: 'Nanjing', z: '南京', r : 'Нанкин'} , 
						  Hangzhou 	: { e: 'Hangzhou', z: '杭州', r : 'Ханчжоу'} , 
						  Datong  	: { e: 'Datong', z: '大同', r : 'Датун'} , 
						  Kunming 	: { e: 'Kunming', z: '昆明', r : 'Куньмин'} ,
						  Sanya 	: { e: 'Sanya', z: '三亚', r : 'Санья'} ,
						  Chongqing : { e: 'Chongqing', z: '重庆', r : 'Чунцин'},
						  Harbin 	: { e: 'Harbin', z: '哈尔滨', r : 'Харбин'} ,
						  Hongkong 	: { e: 'Hongkong', z: '香港', r : 'Гонконг'} , 
						  Tianjin 	: { e: 'Tianjin', z: '天津', r : 'Тяньцзинь'} ,
						  Urumqi 	: { e: 'Urumqi', z: '乌鲁木齐', r : 'Урумчи'}}; 
			var russiaCity ={ spb 			: { e: 'Saint Petersburg', z: '圣彼得堡', r : 'Санкт-Петербург'} , 
						  moscow 			: { e: 'Moscow', z: '莫斯科', r : 'Москва'} ,
						  yekaterinburg  	: { e: 'Yekaterinburg', z: '叶卡捷琳堡', r : 'Екатеринбург'} , 
						  novosibirsk 		: { e: 'Novosibirsk', z: '新西伯利亚', r : 'Новосибирск'} ,
						  omsk 				: { e: 'Omsk', z: '鄂木斯克', r : 'Омск'} , 
						  kazan 		: { e: 'Kazan', z: '喀山', r : 'Казань'} ,
						  ufa 			: { e: 'Ufa', z: '乌法', r : 'Уфа'} , 
						  krasnoyarsk 	: { e: 'Krasnoyarsk', z: '克拉斯诺亚尔斯克', r : 'Красноярск'} ,
						  khabarovsk 	: { e: 'Khabarovsk', z: '伯力', r : 'Хабаровск'} , 
						  vladivostok 	: { e: 'Vladivostok', z: '海参崴', r : 'Владивосток'} ,
						  chita 		: { e: 'Chita', z: '赤塔', r : 'Чита'} , 
						  yakutsk 		: { e: 'Yakutsk', z: '雅库茨克', r : 'Якутск'} ,
						  arkhangelsk 	: { e: 'Arkhangelsk', z: '阿尔汉格尔斯克', r : 'Архангельск'} , 
						  volgograd 		: { e: 'Volgograd', z: '伏尔加格勒', r : 'Волгоград'} ,
						  novgorod 	: { e: 'Veliky Novgorod', z: '大诺夫哥罗德', r : 'Великий Новгород'}};
			var img_path = current_obj.currentImg.find("img").attr("src").split("/")
			var img_path_size = img_path.length;
			var img_name = img_path[img_path_size-1]
			img_name = img_name.split(".")[0]
			var cities;
			if(current_obj.currentImg.parents("div").filter(".popUpChinaCity").length != 0) { cities = chinaCity; }
			else { cities = russiaCity; }
			$.each( cities, function( k, v){
				if( k == img_name){
					//console.log(img_name);
					var img_name_div = '<div style="float:left; width:33%; text-align:center; color:#990000; font-weight: bold;">' + v.z + '</div>'
					img_name_div = img_name_div + '<div style="float:left; width:33%; text-align:center; color:#990000; font-weight: bold;">' + v.e + '</div>'
					img_name_div = img_name_div + '<div style="float:left; width:33%; text-align:center; color:#990000; font-weight: bold;">' + v.r + '</div>'
					allinone_carousel_playOver.html("<br><br>")
					allinone_carousel_playOver.html(allinone_carousel_playOver.html() + img_name_div)
					newStyle = allinone_carousel_playOver.attr("style").replace("none","block")
					allinone_carousel_playOver.attr("style", newStyle)
				}
			})
			// end
	...
	$.fn.allinone_carousel = function(options) {
		...
		return this.each(function() {
			...
			contentHolderUnit.click(function() {
				...
				if (true) {
					...
					if (i!=current_obj.current_img_no) {
						...
						//console.log(i+'  --  '+current_obj.current_img_no+'  --  '+total_images);
						//deactivate previous 
						$(bottomNavButs[current_obj.current_img_no]).removeClass('bottomNavButtonON');	
						...
						//begin
						i = parseInt(i);
						var current_img_no = current_obj.current_img_no
						current_obj.fastForwardRunning=true;
						options.animationTime=0;
						options.autoPlay=0.1;
						current_obj.img_no_where_to_stop=i;						
						...
						//console.log("判断点击左右按钮","差："+(current_img_no-i)+" i:"+i+" current_img_no:"+current_img_no);
						if(current_img_no-i>1){
							//console.log("模拟向左按钮点击事件");
							setTimeout("$('.leftNav').trigger('click');",500);
						}
						
						if(i-current_img_no>1){
							//console.log("模拟向右按钮点击事件");
							setTimeout("$('.rightNav').trigger('click');",500);
						}
						//end
			...
			//controllers
			allinone_carousel_leftNav.click(function() {
				//if (!current_obj.slideIsRunning && !current_obj.fastForwardRunning) {
					//current_obj.mouseOverBanner=false;
					current_obj.isVideoPlaying=false;
					clearTimeout(current_obj.timeoutID);
			...
			allinone_carousel_rightNav.click(function() {
				//if (!current_obj.slideIsRunning && !current_obj.fastForwardRunning) {
					//current_obj.mouseOverBanner=false;
					current_obj.isVideoPlaying=false;
					clearTimeout(current_obj.timeoutID);
