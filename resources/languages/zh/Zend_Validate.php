...
return array(
    ...
    "'%value%' contains characters which are non alphabetic and no digits" => "'%value%' 密码中只能包含字母和数字",
    "'%value%' is an empty string" => "'%value%' is an empty string",
    "Почтовый адрес '%value%' уже используется" => "'%value%'用户名已经被使用，请你使用新的用户名",
    // Zend_Validate_Alpha
    ...
    "Invalid type given. String expected" => "Invalid type given. String expected",
    "'%value%' is no valid email address in the basic format local-part@hostname" => "'%value%' 这个邮箱地址不正确请参考 local-part@hostname",
    ...
    "Invalid type given. String expected" => "Invalid type given. String expected",
    "'%value%' is less than %min% characters long" => "'%value%' 不能小于 %min% 字符",
